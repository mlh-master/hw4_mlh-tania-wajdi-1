...
# applying the model to the test
loss_and_metrics = model_relu.evaluate(X_test, Y_test, verbose=2)
# Printing the accuracy and loss
print('Accuracy over the testing set is',str(loss_and_metrics[1]),', Loss over the testing set is',str(loss_and_metrics[0]))
#----------------------------------------------------------------------------------------
# -
...
if not("results" in os.listdir()):
    os.mkdir("results")
save_dir = "results/"
model_name = "initial_weights.25"
model_path = os.path.join(save_dir, model_name)
new_a_model.save(model_path)
print('Saved trained model at %s ' % model_path)
# model training
new_a_history1 = new_a_model.fit(BaseX_train,BaseY_train,batch_size=batch_size,epochs=epochs)
# applying the model to the test
loss_and_metrics_a1 = new_a_model.evaluate(X_test, Y_test, verbose=2)
# Printing the accuracy and loss
print('Accuracy over the testing set is',str(loss_and_metrics_a1[1]),', Loss over the testing set is',str(loss_and_metrics_a1[0]))
#-----------------------------------------------------------------------------------------
...
# +
#--------------------------Impelment your code here:-------------------------------------
#loading the saved model - same weights that we used in 25 epochs
saved_new_a_model = load_model("results/initial_weights.25")
# model training
new_a_history2 = saved_new_a_model.fit(BaseX_train,BaseY_train,batch_size=batch_size,epochs=epochs)
# applying the model to the test
loss_and_metrics_a2 = saved_new_a_model.evaluate(X_test, Y_test, verbose=2)
# Printing the accuracy and loss
print('Accuracy over the testing set is',str(loss_and_metrics_a2[1]),', Loss over the testing set is',str(loss_and_metrics_a2[0]))
#-----------------------------------------------------------------------------------------
# -
...
new_a_model.compile(optimizer=AdamOpt, metrics=['accuracy'], loss='categorical_crossentropy')
# model training
new_a_history3 = new_a_model.fit(BaseX_train,BaseY_train,batch_size=batch_size,epochs=epochs)
# applying the model to the test
loss_and_metrics_a3 = new_a_model.evaluate(X_test, Y_test, verbose=2)
# Printing the accuracy and loss
print('Accuracy over the testing set is',str(loss_and_metrics_a3[1]),', Loss over the testing set is',str(loss_and_metrics_a3[0]))
#----------------------------------------------------------------------------------------
# -
...
# +
#--------------------------Impelment your code here:-------------------------------------
def get_net(input_shape,drop,dropRate,reg,filter_lst):
    #Defining the network architecture:
    model = Sequential()
    model.add(Permute((1,2,3),input_shape = input_shape))
    model.add(Conv2D(filters=int(filter_lst[0]), kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_1',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Conv2D(filters=int(filter_lst[1]), kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_2',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(Conv2D(filters=int(filter_lst[2]), kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_3',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Conv2D(filters=int(filter_lst[3]), kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_4',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(Conv2D(filters=int(filter_lst[4]), kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_5',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    ...
    model.summary()
    return model
filt_lst = np.array([64, 128, 128, 256, 256])
new_filt_lst = filt_lst/2
# new_filt_lst = np.array([32, 64, 64, 128, 128])
NNet_FR = get_net(input_shape,drop,dropRate,reg,new_filt_lst) #FR = Filter Reduced
AdamOpt = Adam(lr=learn_rate,decay=decay)
NNet_FR.compile(optimizer=AdamOpt, metrics=['acc'], loss='categorical_crossentropy')
h_FR = NNet_FR.fit(x=BaseX_train, y=BaseY_train, batch_size=batch_size, epochs=epochs, verbose=1, validation_split=0, validation_data = (BaseX_val, BaseY_val), shuffle=True)
results_FR = NNet_FR.evaluate(X_test,Y_test)
print('test loss, test acc:', results_FR)
#----------------------------------------------------------------------------------------
# -
